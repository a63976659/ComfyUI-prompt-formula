...
class 视频转场基类:
    ...
    def _generate_full_prompt_with_visual_consistency(self, 首帧, 尾帧, 转场, 视觉连贯性, 附加描述=""):
        """生成完整提示词（将尾帧描述放在视觉连贯性描述的后面）"""
        组件 = []
        ...
        if 转场:
            ...
            # 添加视觉连贯性描述（如果有转场且有视觉连贯性）
            if 视觉连贯性 and 视觉连贯性 != "无":
                连贯性描述 = self._get_visual_consistency_description(视觉连贯性)
                转场组件.append(连贯性描述)
    ...
    def _generate_occlusion_transition(self, 遮挡类型, 时长, 节奏, 运动子类型="无", 运动方向="无", 前景遮挡物=None):
        """生成遮挡物转场提示词 - 修改版：使用前景遮挡物组件输入"""
        ...
        if 遮挡类型 == "前景物体遮挡":
            # 优先使用前景遮挡物组件输入
            if 前景遮挡物:
                遮挡物名称 = clean_text(前景遮挡物)
            
            # 如果没有提供前景遮挡物，则使用默认的遮挡物名称
            if not 遮挡物名称:
                遮挡物名称 = self._get_default_occlusion_name(遮挡类型)
        else:
            # 对于其他遮挡类型，使用常量配置获取默认的遮挡物名称
            遮挡物名称 = self._get_default_occlusion_name(遮挡类型)
    ...
    def _generate_unfold_transition(self, 展开方式, 时长, 节奏, 前景遮挡物=None, 默认主体="主体"):
        """生成画卷展开式转场提示词"""
        # 获取展开的目标
        展开目标 = 前景遮挡物 if 前景遮挡物 else 默认主体
        
        # 根据展开方式生成对应的描述
        展开描述映射 = {
            "人物展开": f"{展开目标}走到镜头前向两边分散开，画面也从中间向两边展开过渡到下一个画面",
            "物体展开": f"{展开目标}在镜头前向两边展开，画面也随之从中间向两边展开过渡到下一个画面",
            "画卷展开": f"{展开目标}像画卷一样在镜头前向两边展开，画面也从中间向两边展开过渡到下一个画面",
            "门窗展开": f"{展开目标}像门窗一样向两边打开，画面也随之展开过渡到下一个画面",
            "对称展开": f"{展开目标}以画面中心为轴心向两边对称展开，画面也从中间向两边展开过渡",
        }
        
        展开描述 = 展开描述映射.get(展开方式, f"{展开目标}在镜头前向两边展开，画面也从中间向两边展开过渡到下一个画面")
        
        return f"{展开描述}，历时{时长}秒，{节奏}"
...
class 视频首尾帧转场(视频转场基类):
    ...
    def 生成转场提示词(self, 首帧描述_建议填写, 尾帧描述_建议填写, 主要转场方式, 转场时长, 运动平滑度,
        ...
        try:
            ...
            转场提示词 = self._generate_transition_prompt(
                ...
                首帧主体_清理, 尾帧主体_清理, 前景遮挡物_清理, 展开方式
            )
            
            # 生成完整提示词 - 修改：将尾帧描述放在视觉连贯性描述的后面
            完整提示词 = self._generate_full_prompt_with_visual_consistency(
                首帧描述_建议填写_清理, 尾帧描述_建议填写_清理, 转场提示词, 视觉连贯性, 附加描述_清理
            )
            
            # 生成技术说明
            技术说明 = self._generate_technical_note(
                主要转场方式, 转场时长, 运动平滑度, 转场节奏, 视觉连贯性,
                首帧主体_清理, 尾帧主体_清理, 人物变化类型, 前景遮挡物_清理, 展开方式
            )
    ...
    def _generate_transition_prompt(self, 主要方式, 运动子类型, 运动方向, 变形子类型, 
                                  ...
                                  首帧主体, 尾帧主体, 前景遮挡物=None, 展开方式="画卷展开"):
        """生成具体的转场提示词"""
        ...
        if 主要方式 == "运镜提示词转场":
            ...
            # 修改：传递前景遮挡物参数
            基础提示词 = self._generate_occlusion_transition(遮挡物类型, 时长格式化, 节奏描述, 运动子类型, 运动方向, 前景遮挡物)
            ...
            基础提示词 = self._generate_unfold_transition(展开方式, 时长格式化, 节奏描述, 前景遮挡物, 首帧主体)
        
        else:
            基础提示词 = ""

        # 注意：视觉连贯性描述现在在完整提示词生成时处理，不在转场提示词中添加
        return 基础提示词
    ...
    def _generate_technical_note(self, 主要方式, 时长, 平滑度, 节奏, 连贯性, 首帧主体, 尾帧主体, 人物变化类型="", 前景遮挡物="", 展开方式=""):
        """生成技术说明"""
        技术要点 = []
        ...
        # 前景遮挡物信息（如果提供了）
        if 前景遮挡物:
            技术要点.append(f"前景遮挡物: {前景遮挡物}")
        
        # 展开方式信息（如果是画卷展开式转场）
        if 主要方式 == "画卷展开式转场" and 展开方式:
            技术要点.append(f"展开方式: {展开方式}")
...
class 视频首尾帧转场_增强版(视频转场基类):
    ...
    def 生成转场提示词(self, 首帧描述_建议填写, 尾帧描述_建议填写, 主要转场方式, 转场时长, 运动平滑度,
        ...
        try:
            ...
            转场提示词 = self._generate_transition_prompt(
                主要转场方式, 运动子类型, 运动方向, 变形子类型, 遮挡物类型,
                人物变化类型, 转场时长, 运动平滑度, 转场节奏, 视觉连贯性,
                首帧主体_清理, 尾帧主体_清理, 运镜方式, 前景遮挡物_清理, 展开方式
            )
            ...
            # 修改：使用新的完整提示词生成方法，将尾帧描述放在视觉连贯性描述的后面
            完整提示词 = self._generate_full_prompt_with_effects_and_visual_consistency(
                首帧描述_建议填写_清理, 尾帧描述_建议填写_清理, 
                转场提示词, 视觉连贯性, 人物效果提示词, 环境效果提示词, 附加描述_清理
            )
            ...
            技术说明 = self._generate_technical_note_enhanced(
                ...
                运镜方式, 人物动态效果, 人物效果强度, 环境动态效果, 环境效果强度,
                前景遮挡物_清理, 展开方式  # 新增参数
            )
    ...
    def _generate_transition_prompt(self, 主要方式, 运动子类型, 运动方向, 变形子类型, 
                                  遮挡物类型, 人物变化类型, 时长, 平滑度, 节奏, 连贯性,
                                  首帧主体, 尾帧主体, 运镜方式="无", 前景遮挡物=None, 展开方式="画卷展开"):
        """生成具体的转场提示词"""
    ...
    def _generate_full_prompt_with_effects_and_visual_consistency(self, 首帧, 尾帧, 转场, 视觉连贯性, 
                                                                人物效果="", 环境效果="", 附加描述=""):
        """生成完整提示词（将尾帧描述放在视觉连贯性描述的后面，带动态效果）"""
        组件 = []
...
class 视频运镜提示词:
    ...
    def 生成运镜提示词(self, 运镜方式, 镜头目标, 运镜速度, 运镜时长, 画面动态描述=""):
        
        try:
            # 清理输入文本
            镜头目标_清理 = clean_text(镜头目标) or "主体"
            附加描述_清理 = clean_text(画面动态描述)
            
            # 格式化时长，保留一位小数
            运镜时长格式化 = f"{运镜时长:.1f}"
            
            # 生成基础运镜提示词
            基础运镜提示词 = self._generate_camera_movement_prompt(
                运镜方式, 镜头目标_清理, 运镜速度, 运镜时长格式化
            )
            
            # 生成完整提示词：将画面动态描述放在前面
            if 附加描述_清理 and 基础运镜提示词:
                运镜提示词 = f"{附加描述_清理}，{基础运镜提示词}"
            elif 附加描述_清理:
                运镜提示词 = 附加描述_清理
            else:
                运镜提示词 = 基础运镜提示词
