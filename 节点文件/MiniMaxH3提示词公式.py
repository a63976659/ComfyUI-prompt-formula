...
def _获取H3运镜描述(运镜方式, 视频时长=None):
    """获取运镜方式的自然语言描述（复用现有运镜描述映射），传入视频时长时要求运镜在时长中间完成"""
    ...
    if 视频时长 is not None:
        时长格式化 = f"{视频时长:.0f}"
        运镜描述 = f"{运镜描述}，运镜过程安排在视频时长的中间段完成，起始与结束画面保持稳定。"
        return 运镜描述
...
H3_BGM_OPTIONS = ["按声音设计描述", "按剧情和画面自动适配", "不需要背景音乐"]
...
def _获取背景音乐文本(背景音乐):
    """获取背景音乐选项对应的声明文本"""
    if 背景音乐 == "不需要背景音乐":
        return H3_BGM_NONE_TEXT
    if 背景音乐 == "按剧情和画面自动适配":
        return H3_BGM_AUTO_TEXT
    return ""


def _获取H3转场文本(转场方式, 转场时长):
    """生成转场描述文本（复用视频首尾帧转场节点的转场选项）"""
    if not 转场方式 or 转场方式 == "无":
        return ""
    时长格式化 = f"{转场时长:.0f}"
    return (f"转场方式：使用{转场方式}，转场时长约{时长格式化}秒，"
            f"转场过程平滑自然，保持首尾画面视觉连贯。")


def _获取H3首尾帧转场文本(转场方式, 转场时长, 视频时长):
    """生成首尾帧节点的转场描述文本，要求转场过程安排在视频时长的中间"""
    if not 转场方式 or 转场方式 == "无":
        return ""
    转场时长格式化 = f"{转场时长:.0f}"
    视频时长格式化 = f"{视频时长:.0f}"
    return (f"转场方式：使用{转场方式}，转场时长约{转场时长格式化}秒，"
            f"转场过程安排在{视频时长格式化}秒视频时长的中间段进行，"
            f"前段充分展示首帧画面，后段充分展示尾帧画面，"
            f"转场平滑自然，保持首尾画面视觉连贯。")


def _解析时间值(值):
    """解析镜头时间为浮点秒数；兼容旧版字符串格式（如'5秒'），无法解析时返回0.0"""
    try:
        return float(值)
    except (TypeError, ValueError):
        pass
    匹配 = re.search(r'-?\d+(?:\.\d+)?', str(值))
    return float(匹配.group()) if 匹配 else 0.0
...
class H3文生视频公式:
    ...
    DESCRIPTION = ("MiniMax H3 纯文生视频提示词公式。可用于 MiniMax API 节点（模型名 MiniMax-H3），提示词不超过7000字符，视频生成为异步任务。\n"
                   ...
                   "会自动写入官方推荐格式'非叙事性音乐：N/A'；选'按剧情和画面自动适配'则写入自动适配声明。\n"
                   "【转场设计】与'视频首尾帧转场'节点使用相同的转场选项（运镜提示词转场/交叉溶解转场/形态变形转场等），"
                   "转场方式与时长会写入【整体要求补充】，选'无'则不生成该段落。\n"
    ...
    def 生成H3文生视频提示词(self, 视频时长, 核心创意, 画面过程描述,
                         影像风格="无", 转场方式="无", 转场时长=3.0,
                         声音设计="", 背景音乐="按剧情和画面自动适配", 附加要求=""):
        try:
            # 清理输入文本
...
class H3首尾帧公式:
    ...
    DESCRIPTION = ("MiniMax H3 首帧/尾帧图生视频提示词公式。实际图片请在 MiniMax API 节点中分别以 first_frame / last_frame 传入（0~2张）。\n"
                   ...
                   "选择运镜后会自动要求运镜过程在视频时长的中间段完成。\n"
                   "【声音设计】H3输出自带原生立体声，音乐/音效/人声统一在此描述："
                   "背景音乐（如'轻柔钢琴曲，缓慢节奏'）、环境音效（如'风声、鸟鸣、衣料摩擦声'）、"
                   "人物台词与音色（如'女主轻声说：……'，TTS支持中英日韩法德西等11种语言）。留空则不生成该段落。\n"
                   "【背景音乐】留空声音设计≠无BGM，H3默认会生成声音；明确不要背景音乐时选'不需要背景音乐'，"
                   "会自动写入官方推荐格式'非叙事性音乐：N/A'；选'按剧情和画面自动适配'则写入自动适配声明。\n"
                   "【转场设计】与'视频首尾帧转场'节点使用相同的转场选项，按所选转场方式与时长完成首帧到尾帧的演变，"
                   "转场过程自动安排在视频时长的中间段，前段留足首帧展示、后段留足尾帧展示；"
                   "选'无'则不生成转场段落。")

    def 生成H3首尾帧提示词(self, 视频时长, 首帧画面描述, 尾帧画面描述, 演变过程描述,
                        运镜方式="无", 转场方式="无", 转场时长=3.0,
                        声音设计="", 背景音乐="按剧情和画面自动适配"):
        try:
            # 清理输入文本
            ...
            # 运镜描述（要求运镜过程在视频时长中间完成）
            运镜描述 = _获取H3运镜描述(运镜方式, 视频时长)
            if 运镜描述:
                画面组件.append(f"运镜：{运镜描述}")

            # 转场描述（与视频首尾帧转场节点相同的可选项，转场过程安排在视频时长中间）
            转场文本 = _获取H3首尾帧转场文本(转场方式, 转场时长, 视频时长)
            if 转场文本:
                画面组件.append(转场文本)
...
class H3图生视频公式:
    ...
    def 生成H3图生视频提示词(self, 视频时长, 源图像描述, 动态剧情描述,
                         运镜方式="无", 影像风格="无", 声音设计="", 背景音乐="按剧情和画面自动适配"):
        try:
            # 清理输入文本
...
class H3音画多镜头公式:
    ...
    最大镜头数 = 50
    镜头组件字段 = [
        ("开始时间", "FLOAT", {"default": 0, "min": 0, "max": 100, "step": 0.5, "round": 0.5, "display": "slider"}),
        ("结束时间", "FLOAT", {"default": 0, "min": 0, "max": 100, "step": 0.5, "round": 0.5, "display": "slider"}),
        ("景别", "COMBO", {"default": "无"}),
        ("描述", "STRING", {"multiline": True, "default": ""}),
        ("运镜方式", "COMBO", {"default": "无"}),
        ("台词", "STRING", {"multiline": True, "default": ""}),
        ("声音设计", "STRING", {"multiline": True, "default": ""})
    ]
    # 前3个镜头的示例默认值（其余镜头使用通用默认值）
    镜头默认值 = {
        1: {"开始时间": 0, "结束时间": 5, "景别": "近景",
            "描述": "侠客推门而入，雨水顺着斗笠滴落，目光扫过店内众人。",
            "运镜方式": "无", "台词": "", "声音设计": "雨声、鼓点骤起"},
        2: {"开始时间": 5, "结束时间": 10, "景别": "特写",
            "描述": "侠客手部特写，缓缓按住腰间刀柄，指节微微收紧。",
            "运镜方式": "推近镜头", "台词": "", "声音设计": "雨声、鼓点骤起"},
        3: {"开始时间": 10, "结束时间": 15, "景别": "中景",
            "描述": "角落中的黑衣人起身，烛光在两人之间摇晃，气氛紧绷。",
            "运镜方式": "固定镜头", "台词": "", "声音设计": "雨声、鼓点骤起"},
    }
    ...
    def INPUT_TYPES(cls):
        ...
        # 镜头槽位：组件数量与示例默认值跟随镜头数量滑块显隐（参照全能参考素材槽位的生成方式）
        optional = {}
        for 序号 in range(1, cls.最大镜头数 + 1):
            默认 = cls.镜头默认值.get(序号, {})
            for 字段名, 字段类型, 基础配置 in cls.镜头组件字段:
                配置 = dict(基础配置)
                if 字段名 in 默认:
                    配置["default"] = 默认[字段名]
                if 字段类型 == "COMBO":
                    选项 = H3_SHOT_TYPES if 字段名 == "景别" else CAMERA_MOVEMENTS
                    optional[f"镜头{序号}{字段名}"] = (选项, {**配置, "display_name": f"镜头{序号}{字段名}"})
                else:
                    optional[f"镜头{序号}{字段名}"] = (字段类型, {**配置, "display_name": f"镜头{序号}{字段名}"})

        # 通用参数
        optional["转场效果"] = (H3_TRANSITIONS, {"default": "硬切", "display_name": "转场效果"})
        optional["影像风格"] = (H3_IMAGE_STYLES, {
            "default": "无",
            "display_name": "影像风格"
        })
    ...
    DESCRIPTION = ("MiniMax H3 音画多镜头提示词公式。利用H3原生多镜头建模与音画同步能力，每个镜头可单独设计声音。可用于 MiniMax API 节点（模型名 MiniMax-H3）。\n"
                   "【镜头数量】支持1~50个镜头，超出数量的镜头组件会自动隐藏，节点高度随可见镜头数量自动缩放。\n"
                   "【开始/结束时间】单位为秒，支持0.5秒步进；结束时间大于0时才会写入时间戳。\n"
    ...
    def 生成H3音画多镜头提示词(self, 总体描述, 镜头数量, 视频总时长,
        ...
        try:
            ...
            # 汇总镜头数据（按镜头数量滑块动态收集，上限50个）
            原始镜头数据 = []
            for 序号 in range(1, self.最大镜头数 + 1):
                原始镜头数据.append({
                    "序号": 序号,
                    "开始": 镜头输入.get(f"镜头{序号}开始时间", 0),
                    "结束": 镜头输入.get(f"镜头{序号}结束时间", 0),
                    "景别": 镜头输入.get(f"镜头{序号}景别", "无"),
                    "描述": 镜头输入.get(f"镜头{序号}描述", ""),
                    "运镜": 镜头输入.get(f"镜头{序号}运镜方式", "无"),
                    "台词": 镜头输入.get(f"镜头{序号}台词", ""),
                    "声音": 镜头输入.get(f"镜头{序号}声音设计", "")
                })
            ...
            for 镜头 in 原始镜头数据[:int(镜头数量)]:
                ...
                镜头数据.append({
                    "序号": 镜头["序号"],
                    "开始": f"{开始值:g}" if (开始值 or 结束值) else "",
                    "结束": f"{结束值:g}" if 结束值 else "",
                    "景别": 镜头["景别"],
                    "描述": 描述_清理,
                    "运镜": 镜头["运镜"],
                    "台词": clean_text(镜头["台词"]),
    ...
    def _生成镜头结构表(self, 镜头数据, 转场效果):
        ...
        for 镜头 in 镜头数据:
            时间范围 = f"{镜头['开始']}~{镜头['结束']}秒" if 镜头["开始"] and 镜头["结束"] else "时间未定"
