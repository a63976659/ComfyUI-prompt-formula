...
try:
    from 节点文件.工具节点 import (
        ...
        图像输入反转,
        合并多组提示词,
        提取视频帧, # 修改: 提取视频结束帧 -> 提取视频帧
        判断并输出加载的图像,
        批量输出同名图像 # 修改: 批量判断并输出同名图像 -> 批量输出同名图像
    )
except Exception as e:
    ...
    字符串输入反转 = 图像输入反转 = 合并多组提示词 = 提取视频帧 = 判断并输出加载的图像 = 批量输出同名图像 = 工具节点占位符

# 导入 图像遮罩预览节点 (New!)
try:
    from 节点文件.图像遮罩预览 import 图像遮罩预览节点
except Exception as e:
    logging.error(f"🔴 导入图像遮罩预览节点失败 (Import Failed): {str(e)}")
    class 图像遮罩预览占位符:
        @classmethod
        def INPUT_TYPES(cls):
            return {"required": {}}
...
# 导入MiniMax H3提示词公式节点
try:
    from 节点文件.MiniMaxH3提示词公式 import (
        H3文生视频公式,
        H3首尾帧公式,
        H3图生视频公式,
        H3全能参考公式,
...
NODE_DISPLAY_NAME_MAPPINGS = {
    ...
    # MiniMax H3节点
    "H3文生视频公式": "🎬 MiniMax H3 文生视频公式",
    "H3首尾帧公式": "🎬 MiniMax H3 首尾帧公式",
